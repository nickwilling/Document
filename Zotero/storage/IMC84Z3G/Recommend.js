var ipUrl = "ishufang.cnki.net";//此处的ip是Krs发布机器的ip地址  ，外网发布时则修改为域名。
var indexArray = ["//www.cnki.net"];
var ISIE;
...
function RecommendFun() {
    ...
    //写行为记录
    WriteActionLog(userName, cnkiUserKey, window.location, document.referrer ? document.referrer.replace("#", "") : "");

    //记录为我推荐点击
    //WriteClickLog();
    var dbCode = GetQueryStringByName(window.location.href, "dbPrefix") || GetQueryStringByName(window.location.href, "code");
    var sql = $("#SqlValue").val() || $("#sqlVal").val();
...
function WriteActionLog(userName, cnkiUserKey, curUrl, referUrl) {
    ...
    var arr = ipUrl.split(",");
    referUrl = referUrl ? escape(referUrl).replace("%23", "") : "";
    for (var i in arr) {
        var serverUrl = "//" + arr[i] + "/KRS/KRSWriteHandler.ashx";
        var geturl = serverUrl + "?curUrl=" + escape(curUrl) + "&referUrl=" + referUrl + "&cnkiUserKey=" + escape(cnkiUserKey) + "&action=file" + "&userName=" + escape(userName);
        Ajax(geturl);
    }
}
function WriteSQLLog(userName, cnkiUserKey, dbCode, sql) {
    ...
    var arr = ipUrl.split(",");
    var serverUrl = "//" + arr[0] + "/KRS/KRSWriteHandler.ashx";
...
function GetUserName() {
    function getUInfoFromECP(o,usertype) {
        ...
        if (ecpjosntype.UserType.toLowerCase() != usertype)
            return "";
        switch (o) {
            case "uname":
    ...
    return getUInfoFromECP("uname","jf");
}
...
function SetCookie(name, value, expiredays) {
    if (GetCookie(name) == "" || GetCookie(name) == null) {
        ...
        document.cookie = name + "=" + encodeURIComponent(value) + (document.location.href.toLowerCase().indexOf('cnki.net') > 0 ?
...
function SubmitExportAnaly(action, fileNameItems) {
    var arr = ipUrl.split(",");
    for (var i in arr) {
        var serverUrl = "//" + arr[i] + "/KRS/ExportAnal.aspx";
        var parameter = "?action=" + escape(action) + "&cnkiUserKey=" + escape(GetCookie("cnkiUserKey")) + "&fileNameItems=" + escape(fileNameItems);
        Ajax(serverUrl, parameter, "post");
    }
}
...
//        var arr = ipUrl.split(",");
//        for (var i in arr) {
//            var serverUrl = "//" + arr[i] + "/KRS/ClickHandler.ashx";
...
function Ajax(url, parameter, type, callback) {
    ...
    var ie789 = navigator.appName == "Microsoft Internet Explorer" &&
       (navigator.appVersion.indexOf("MSIE 7.0") > -1 || navigator.appVersion.indexOf("MSIE 8.0") > -1 || navigator.appVersion.indexOf("MSIE 9.0") > -1) ? true : false;
